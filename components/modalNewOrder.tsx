...
const ModalNewOrder = () => {
  ...
  const { userId, driverLat, driverLng, currentOrder } = useAppSelector(
    (state) => state.System
  );
    ...
    const onNewOrder = onChildAdded(ordersRef, async (snapshot) => {
      ...
      if ((orderData.assignforuserid = userId)) {
        ...
        const orderObject = {
          ...
          fee: parseFloat(fee),
          feeForDeliver: parseFloat(feeForDeliver),
          instructionsDeliver,
          instructionsRes,
          orderTime,
          orderType,
          ...
          restaurantname,
          restaurantphone,
          restaurantshortaddress: restaurantshortaddress ?? restaurantaddress,
          tipForDeliver: parseFloat(tipForDeliver),
  ...
  useEffect(() => {
    if (
      newOrder &&
      !isFetchingLocation &&
      newOrder.restaurantLat &&
      newOrder.restaurantLng
    ) {
  ...
  }, [newOrder?.Id, newOrder?.distance, newOrder?.duration]);
    ...
    try {
      ...
      if (
        ...
        const distanceInMiles = parseFloat(
          firstLeg.distance.text.replace(" mi", "")
        );
        ...
        const routeCoordinates = points
          ...
          .filter((point) => point !== null); // Filter out any null/invalid points
              ...
              <View style={styles.totalRow}>
                ...
                <View style={styles.totalRight}>
                  <Text style={styles.summaryText}>
                    {newOrder.ordertextinfoforappObject.SubTotal}
                  </Text>
                </View>
              </View>
              <View style={styles.totalRow}>
                ...
                <View style={styles.totalRight}>
                  <Text style={styles.summaryText}>
                    {newOrder.ordertextinfoforappObject.Tax}
                  </Text>
                </View>
              </View>
              {newOrder.ordertextinfoforappObject.Promotion && (
                <View style={styles.totalRow}>
                  <View style={styles.totalLeft}>
